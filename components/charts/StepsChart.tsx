...
export default function StepsChart({ data }: StepsChartProps) {
  ...
  return (
    <div className="w-full h-64 space-y-2">
      <div className="text-xs font-semibold text-text-muted">Daily Steps</div>
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} style={{ background: 'transparent' }}>
          ...
          <ReferenceLine
            ...
            strokeDasharray="5 3"
            strokeWidth={1}
            label={{ value: 'Target: 10k', fill: C.primary, fontSize: 9, position: 'top' }}
          />
          <Bar dataKey="steps" name="Steps" fill={C.emerald} radius={[3, 3, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
